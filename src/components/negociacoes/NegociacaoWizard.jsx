...
export default function NegociacaoWizard({ item, clientes, unidades, loteamentos, onSubmit, onCancel, isProcessing, editMode = "wizard" }) {
  const [currentStep, setCurrentStep] = useState(editMode === "resumo" ? 6 : 1);
  ...
  const allImobiliarias = imobiliarias || [];
  const allCorretores = corretores || [];

  const corretoresFiltrados = formData.imobiliaria_id
    ? allCorretores.filter(c => c.imobiliaria_id === formData.imobiliaria_id)
    : allCorretores;
  ...
  useEffect(() => {
    if (formData.imobiliaria_id && formData.valor_total > 0) {
      const imobiliaria = allImobiliarias.find(i => i.id === formData.imobiliaria_id);
  ...
  }, [formData.imobiliaria_id, formData.valor_total, allImobiliarias]);

  useEffect(() => {
    if (formData.corretor_id && formData.valor_total > 0) {
      const corretor = allCorretores.find(c => c.id === formData.corretor_id);
  ...
  }, [formData.corretor_id, formData.valor_total, allCorretores]);
  ...
  }, [currentStep, formData.cliente_id, formData.unidade_id, formData.valor_total, formData.data_inicio, formData.quantidade_parcelas_mensais]);
          ...
          {formData.tipo_correcao === "anual" && (
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setFormData({ ...formData, mes_correcao_anual: parseInt(value) || 1 })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
  ...
  const renderStep5 = () => (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="space-y-2">
        ...
        <Input
          value={allImobiliarias.find(i => i.id === formData.imobiliaria_id)?.nome || ""}
          disabled
          className="bg-gray-100"
          placeholder="Clique na lupa para selecionar..."
        />
    ...
    <>
      <Card className="mb-6 shadow-lg border-t-4 border-[var(--wine-600)]">
        ...
        <CardContent className="p-6">
          {editMode === "resumo" ? (
            // Modo Resumo - Ir direto para o passo 6
        ...
        <CardFooter className="flex justify-between border-t pt-4">
          <Button
            ...
            disabled={isProcessing || (editMode === "resumo" && currentStep === 6)}
          >
