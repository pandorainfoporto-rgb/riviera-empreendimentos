...
export default function PortalClienteDocumentos() {
  ...
  const documentosContratos = documentosFiltrados.filter(d => d.tipo === 'contrato' || d.tipo === 'negociacao');
  const documentosGerais = documentosFiltrados.filter(d => d.tipo === 'documento_geral' || d.tipo === 'projeto');
  const documentosFinanceiros = documentosFiltrados.filter(d => d.tipo === 'nota_fiscal' || d.tipo === 'recibo' || d.tipo === 'pagamento');
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              ...
              {tiposDisponiveis.map(tipo => (
                ...
                >
                  {getTipoLabel(tipo)} ({documentos.filter(d => d.tipo === tipo).length})
                </Button>
              ))}
            </div>
          </div>
                  ...
                  <div key={doc.id} className="p-4 bg-white rounded-lg border hover:shadow-md transition-all">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button size="sm" className="bg-blue-600" onClick={() => window.open(doc.arquivo_url, '_blank')}>
        ...
        <Card className="shadow-lg">
          <CardContent className="text-center py-12">
            ...
            <p className="text-sm text-gray-500 mt-2">
              {searchTerm ? 'Tente ajustar os filtros' : 'Documentos serão adicionados em breve'}
            </p>
          </CardContent>
        </Card>
      )}
