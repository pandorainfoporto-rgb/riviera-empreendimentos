...
import {
  Home, DollarSign, Users, TrendingUp, TrendingDown,
  Calendar, AlertTriangle, CheckCircle2, Building, Eye, Clock, Key
} from "lucide-react";
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip, Legend, BarChart, Bar, XAxis, YAxis, CartesianGrid } from "recharts";
import { format, parseISO, startOfMonth, endOfMonth, eachMonthOfInterval, subMonths, isBefore, isAfter } from "date-fns";
import { ptBR } from "date-fns/locale";
import { Link } from "react-router-dom";
import { createPageUrl } from "@/utils";

export default function DashboardAlugueis() {
  const [periodoMeses, setPeriodoMeses] = useState(6);

  const { data: locacoes = [] } = useQuery({
    queryKey: ['locacoes'],
    queryFn: () => base44.entities.Locacao.list(),
  });
  ...
  const encargosTotal = locacoesAtivas.reduce((sum, l) => 
    sum + (l.valor_condominio || 0) + (l.valor_iptu || 0), 0
  );
  ...
  const meses = eachMonthOfInterval({
    start: startOfMonth(subMonths(hoje, periodoMeses - 1)),
    end: endOfMonth(hoje)
  });
  ...
  const receitaPorMes = meses.map(mes => {
    ...
    return {
      ...
      previsto: locacoesAtivas.length > 0 
        ? locacoesAtivas.reduce((sum, l) => sum + (l.valor_total_mensal || 0), 0)
        : 0,
    };
  });
  ...
  return (
    <div className="p-4 md:p-8 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[var(--wine-700)]">Dashboard de Locações</h1>
          <p className="text-gray-600 mt-1">Visão completa do portfólio de aluguéis</p>
        </div>
      ...
      <div className="grid md:grid-cols-4 gap-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-blue-700">{locacoesAtivas.length}</p>
                <p className="text-xs text-gray-500 mt-1">
                  {locacoes.length} total
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {locacoesAtivas.length} contratos
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-red-700">{alugueisVencidosHoje.length}</p>
                <p className="text-xs text-gray-500 mt-1">
                  Requer atenção
                </p>
              </div>
        ...
        <Card className="border-t-4 border-purple-500">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">Taxa Adimplência</p>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Inquilinos Ativos ({clientesInquilinos.filter(c => 
              locacoesAtivas.some(l => l.cliente_id === c.id)
            ).length})
          </CardTitle>
        </CardHeader>
        <CardContent>
              ...
              return (
                <div key={locacao.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">{unidade?.codigo}</p>
                  </div>
